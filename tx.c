 ...
 *	 undertested.
 */
static u8 mt76x0_tx_pktid_enc(struct mt76x0_dev *dev, u8 rate, bool is_probe)
{
...
static void
mt76x0_tx_pktid_dec(struct mt76x0_dev *dev, struct mt76_tx_status *stat)
{
...
void mt76x0_tx_stat(struct work_struct *work)
{
	...
	while (!test_bit(MT76X0U_STATE_REMOVED, &dev->state)) {
	...
	if (cleaned)
		queue_delayed_work(dev->stat_wq, &dev->stat_work,
				   msecs_to_jiffies(10));
	else if (test_and_clear_bit(MT76X0U_STATE_MORE_STATS, &dev->state))
		queue_delayed_work(dev->stat_wq, &dev->stat_work,
				   msecs_to_jiffies(20));
	else
		clear_bit(MT76X0U_STATE_READING_STATS, &dev->state);
