...
mt76x0_set_wlan_state(struct mt76x0_dev *dev, u32 val, bool enable)
{
	...
	mt76x0_wr(dev, MT_WLAN_FUN_CTRL, val);
	udelay(20);

	if (enable) {
		set_bit(MT76X0U_STATE_WLAN_RUNNING, &dev->state);
	} else {
		clear_bit(MT76X0U_STATE_WLAN_RUNNING, &dev->state);
		return;
	}

	for (i = 200; i; i--) {
		val = mt76x0_rr(dev, MT_CMB_CTRL);
...
static void mt76x0_chip_onoff(struct mt76x0_dev *dev, bool enable, bool reset)
{
	...
	val = mt76x0_rr(dev, MT_WLAN_FUN_CTRL);
...
static void mt76x0_reset_csr_bbp(struct mt76x0_dev *dev)
{
	...
	val = mt76x0_rr(dev, MT_PBF_SYS_CTRL);
	val &= ~0x2000;
	mt76x0_wr(dev, MT_PBF_SYS_CTRL, val);

	mt76x0_wr(dev, MT_MAC_SYS_CTRL, MT_MAC_SYS_CTRL_RESET_CSR |
...
static void mt76x0_init_usb_dma(struct mt76x0_dev *dev)
{
	...
	val = mt76x0_rr(dev, MT_USB_DMA_CFG);
	...
	mt76x0_wr(dev, MT_USB_DMA_CFG, val);

	val = mt76x0_rr(dev, MT_COM_REG0);
	if (val & 1)
		printk("XXX MCU not ready\n");
...
mt76_init_beacon_offsets(struct mt76x0_dev *dev)
{
	...
	for (i = 0; i < 4; i++)
		mt76x0_wr(dev, MT_BCN_OFFSET(i), regs[i]);
}
...
{
	mt76x0_rr(dev, MT_RX_STA_CNT0);
	mt76x0_rr(dev, MT_RX_STA_CNT1);
	mt76x0_rr(dev, MT_RX_STA_CNT2);
	mt76x0_rr(dev, MT_TX_STA_CNT0);
	mt76x0_rr(dev, MT_TX_STA_CNT1);
	mt76x0_rr(dev, MT_TX_STA_CNT2);
}

int mt76x0_mac_start(struct mt76x0_dev *dev)
{
	mt76x0_wr(dev, MT_MAC_SYS_CTRL, MT_MAC_SYS_CTRL_ENABLE_TX);
	...
	mt76x0_wr(dev, MT_RX_FILTR_CFG, dev->rxfilter);

	mt76x0_wr(dev, MT_MAC_SYS_CTRL,
...
static void mt76x0_mac_stop_hw(struct mt76x0_dev *dev)
{
	...
	if (test_bit(MT76X0U_STATE_REMOVED, &dev->state))
		return;
...
int mt76x0_init_hardware(struct mt76x0_dev *dev)
{
	...
	mt76x0_wr(dev, MT_HEADER_TRANS_CTRL_REG, 0x0);
	mt76x0_wr(dev, MT_TSO_CTRL, 0x0);
	...
	mt76x0_rmw(dev, MT_US_CYC_CFG, MT_US_CYC_CNT, 0x1e);

	mt76x0_wr(dev, MT_TXOP_CTRL_CFG,
...
{
	if (!test_and_clear_bit(MT76X0U_STATE_INITIALIZED, &dev->state))
		return;
...
{
	dev->sband_5g = devm_kzalloc(dev->dev, sizeof(*dev->sband_5g),
				     GFP_KERNEL);
...
int mt76x0_register_device(struct mt76x0_dev *dev)
{
	...
	dev->chandef.chan = &dev->sband_2g->channels[0];
