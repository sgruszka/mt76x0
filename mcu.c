...
{
	return mt76x0_rr(dev, MT_MCU_COM_REG0) == 1;
}
...
static inline void trace_mt_mcu_msg_send_cs(struct mt76x0_dev *dev,
			    ...
			    const void *data, u32 len, u32 dst_addr)
{
	...
	val = mt76x0_rr(dev, MT_TX_CPU_FROM_FCE_CPU_DESC_IDX);
	val++;
	mt76x0_wr(dev, MT_TX_CPU_FROM_FCE_CPU_DESC_IDX, val);
...
static int mt76x0_load_firmware(struct mt76x0_dev *dev)
{
	...
	mt76x0_wr(dev, MT_USB_DMA_CFG, (MT_USB_DMA_CFG_RX_BULK_EN |
					 MT_USB_DMA_CFG_TX_BULK_EN));

	if (firmware_running(dev))
		return 0;

	ret = request_firmware(&fw, MT7610_FIRMWARE, dev->dev);
	if (ret)
		return ret;
...
*/
	...
	mt76x0_wr(dev, MT_FCE_PSE_CTRL, 1);

	/* FCE tx_fs_base_ptr */
	mt76x0_wr(dev, MT_TX_CPU_FROM_FCE_BASE_PTR, 0x400230);
	/* FCE tx_fs_max_cnt */
	mt76x0_wr(dev, MT_TX_CPU_FROM_FCE_MAX_COUNT, 1);
	/* FCE pdma enable */
	mt76x0_wr(dev, MT_FCE_PDMA_GLOBAL_CONF, 0x44);
	/* FCE skip_fs_en */
	mt76x0_wr(dev, MT_FCE_SKIP_FS, 3);
...
int mt76x0_mcu_init(struct mt76x0_dev *dev)
{
	...
	set_bit(MT76X0U_STATE_MCU_RUNNING, &dev->state);
