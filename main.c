			 ...
			 struct ieee80211_bss_conf *info, u32 changed)
{
	...
	if (changed & BSS_CHANGED_BASIC_RATES) {
		mt76x0_wr(dev, MT_LEGACY_BASIC_RATE, info->basic_rates);
		mt76x0_wr(dev, MT_HT_FBK_CFG0, 0x65432100);
		mt76x0_wr(dev, MT_HT_FBK_CFG1, 0xedcba980);
		mt76x0_wr(dev, MT_LG_FBK_CFG0, 0xedcba988);
		mt76x0_wr(dev, MT_LG_FBK_CFG1, 0x00002100);
	}
...
static int
mt76_wcid_alloc(struct mt76x0_dev *dev)
{
	int i, idx = 0;
		...
		const u8 *mac_addr)
{
	...
	cancel_delayed_work_sync(&dev->cal_work);
	mt76x0_agc_save(dev);
	set_bit(MT76X0U_STATE_SCANNING, &dev->state);
}
			 ...
			 struct ieee80211_vif *vif)
{
	...
	clear_bit(MT76X0U_STATE_SCANNING, &dev->state);
