...
struct mt76_tx_status mt76x0_mac_fetch_tx_status(struct mt76x0_dev *dev)
{
	...
	stat2 = mt76x0_rr(dev, MT_TX_STAT_FIFO_EXT);
	stat1 = mt76x0_rr(dev, MT_TX_STAT_FIFO);
				...
				int ht_mode)
{
	...
	for (i = 0; i < 6; i++)
		mt76x0_wr(dev, MT_CCK_PROT_CFG + i * 4, prot[i]);
}
...
{
	u32 val = mt76x0_rr(dev, MT_BEACON_TIME_CFG);
	...
	if (!enable) {
		mt76x0_wr(dev, MT_BEACON_TIME_CFG, val);
		return;
	}
...
void mt76x0_mac_work(struct work_struct *work)
{
	...
	for (i = 0; i < ARRAY_SIZE(spans); i++)
		for (j = 0; j < spans[i].span; j++) {
			u32 val = mt76x0_rr(dev, spans[i].addr_base + j * 4);
...
void mt76x0_mac_set_ampdu_factor(struct mt76x0_dev *dev)
{
	...
	mt76x0_wr(dev, MT_MAX_LEN_CFG, 0xa0fff |
			  ...
			  struct ieee80211_key_conf *key)
{
	...
	mt76x0_wr_copy(dev, MT_WCID_KEY(idx), key_data, sizeof(key_data));
	...
	mt76x0_wr_copy(dev, MT_WCID_IV(idx), iv_data, sizeof(iv_data));

	val = mt76x0_rr(dev, MT_WCID_ATTR(idx));
	...
	mt76x0_wr(dev, MT_WCID_ATTR(idx), val);
			      ...
			      struct ieee80211_key_conf *key)
{
	...
	mt76x0_wr_copy(dev, MT_SKEY(vif_idx, key_idx),
