...
mt76x0_rf_csr_wr(struct mt76x0_dev *dev, u32 offset, u8 value)
{
	...
	if (WARN_ON(!test_bit(MT76X0U_STATE_WLAN_RUNNING, &dev->state)))
		return -EINVAL;
...
out:
	...
	if (ret < 0)
		dev_err(dev->dev, "Error: RF write %d:%d failed:%d!!\n",
			bank, reg, ret);
...
mt76x0_rf_csr_rr(struct mt76x0_dev *dev, u32 offset)
{
	...
	val = mt76x0_rr(dev, MT_RF_CSR_CFG);
...
out:
	...
	if (ret < 0)
		dev_err(dev->dev, "Error: RF read %d:%d failed:%d!!\n",
			bank, reg, ret);
...
{
	if (test_bit(MT76X0U_STATE_MCU_RUNNING, &dev->state)) {
...
int mt76x0_wait_bbp_ready(struct mt76x0_dev *dev)
{
	...
	do {
		val = mt76x0_rr(dev, MT_BBP(CORE, 0));
...
mt76x0_extra_power_over_mac(struct mt76x0_dev *dev)
{
	...
	val = ((mt76x0_rr(dev, MT_TX_PWR_CFG_1) & 0x00003f00) >> 8);
	val |= ((mt76x0_rr(dev, MT_TX_PWR_CFG_2) & 0x00003f00) << 8);
	mt76x0_wr(dev, MT_TX_PWR_CFG_7, val);

	/* TODO: fix VHT */
	val = ((mt76x0_rr(dev, MT_TX_PWR_CFG_3) & 0x0000ff00) >> 8);
	mt76x0_wr(dev, MT_TX_PWR_CFG_8, val);

	val = ((mt76x0_rr(dev, MT_TX_PWR_CFG_4) & 0x0000ff00) >> 8);
	mt76x0_wr(dev, MT_TX_PWR_CFG_9, val);
}
...
mt76x0_phy_set_tx_power(struct mt76x0_dev *dev, u8 channel, u8 rf_bw_band)
{
	...
	for (i = 0; i < 4; i++) {
		...
		mt76x0_wr(dev, MT_TX_PWR_CFG_0 + 4*i, val);
	}
		       ...
		       struct cfg80211_chan_def *chandef)
{
	...
	bool scan = test_bit(MT76X0U_STATE_SCANNING, &dev->state);
...
{
	u32 tx_alc, reg_val;
	u8 channel = dev->chandef.chan->hw_value;
	int is_5ghz = (dev->chandef.chan->band == NL80211_BAND_5GHZ) ? 1 : 0;
	...
	mt76x0_wr(dev, 0x2124, reg_val);
	mt76x0_wr(dev, MT_TX_ALC_CFG_0, tx_alc);
	msleep(100);
...
static void mt76x0_dynamic_vga_tuning(struct mt76x0_dev *dev)
{
	...
	init_vga = (dev->chandef.chan->band == NL80211_BAND_5GHZ) ? 0x54 : 0x4E;
	if (dev->avg_rssi > -60)
		init_vga -= 0x20;
